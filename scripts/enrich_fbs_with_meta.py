...
def load_generated_fbs_data(modelname, output_dir):
    ...
    # Look for parquet files matching the model name in cache
    if os.path.exists(flowsa_cache):
        print(f"Checking FlowSA cache: {flowsa_cache}")
        cache_files = [f for f in os.listdir(flowsa_cache) if f.startswith(modelname) and f.endswith('.parquet')]
    ...
    if os.path.exists(parquet_path):
        ...
        error_msg = [
            ...
            "",
            "The FlowBySector parquet file doesn't exist yet.",
            "Run enrich_fbs_with_meta.py and it will be generated automatically.",
            "First run may take several minutes, then results are cached.",
            "="*80
        ]
...
def generate_flowbysector_data(modelname):
    ...
    This function generates FBS data on the fly. The first run takes several minutes,
    but results are cached by FlowSA for subsequent runs.
    ...
    # Generate FlowBySector data using FlowSA
    # Keep retain_activity_columns=True to preserve detailed activity information
    fbs_calculated = pd.DataFrame(flowsa.flowbysector.FlowBySector.generateFlowBySector(
        modelname,
        download_sources_ok=config.DOWNLOAD_SOURCES_OK,
        retain_activity_columns=config.RETAIN_ACTIVITY_COLUMNS,
        append_sector_names=config.APPEND_SECTOR_NAMES
    ))
    
    print(f"✓ Generated {len(fbs_calculated):,} records using FlowSA")
    
    return fbs_calculated
...
def main():
    ...
    2. Generate or load FlowBySector data  
    ...
    # -------------------------------------------------------------------------
    # STEP 1: Load existing parquet data (optional - for comparison/backup)
    # -------------------------------------------------------------------------
    fbs_parquet = None
    parquet_path = os.path.join(config_dict["input_path"], config_dict["subfolder"], config_dict["file_name_parquet"])
    
    if os.path.exists(parquet_path):
        print("Step 1: Loading existing parquet data...")
        ...
        print("Step 1: Parquet file not found - will generate data using FlowSA")
    
    # -------------------------------------------------------------------------
    # STEP 2: Load pre-generated FlowBySector data
    # -------------------------------------------------------------------------
    print("\nStep 2: Loading pre-generated FlowBySector data...")
    fbs_calculated = load_generated_fbs_data(config_dict["modelname"], config_dict["output_dir"])
...
if __name__ == "__main__":
    # Check for command line arguments to run standalone sanity check
    import sys
    
    if len(sys.argv) > 1 and sys.argv[1] == "--check-only":
        print("="*80)
        print("STANDALONE SANITY CHECK MODE")
        print("="*80)
        print("This will compare a previously generated FBS file with the reference parquet.")
        print()
        
        # Validate FlowSA version first
        validate_flowsa_version()
        
        # Load reference parquet
        parquet_path = os.path.join(
            config.FLOWSA_DATA_PATH, 
            config.FLOWBYSECTOR_SUBFOLDER, 
            config.FILE_NAME_PARQUET
        )
        
        if not os.path.exists(parquet_path):
            print(f"ERROR: Reference parquet not found at: {parquet_path}")
            sys.exit(1)
        
        print("Loading reference parquet...")
        fbs_reference = load_parquet_data(
            config.FLOWSA_DATA_PATH,
            config.FLOWBYSECTOR_SUBFOLDER,
            config.FILE_NAME_PARQUET
        )
        
        # Generate fresh FlowBySector data
        print(f"\nGenerating fresh FlowBySector data for comparison...")
        fbs_calculated = generate_flowbysector_data(config.MODELNAME)
        ...
        # Run comparison
        print("\nRunning comparison...")
        comparison_results = compare_with_reference(fbs_calculated_aggregated, fbs_reference)
        
        # Exit with appropriate code
        if comparison_results["row_count_match"] and comparison_results["data_match"]:
            print("\n✓ Sanity check completed successfully!")
            sys.exit(0)
        else:
            print("\n⚠️  Sanity check found differences - review above.")
            sys.exit(1)
    else:
